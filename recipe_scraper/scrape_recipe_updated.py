...
class RecipeScraper:
    ...
    def parse_ingredient(self, ingredient_li):
        ...
        """
        quantity_span = ingredient_li.find('span', attrs={'data-ingredient-quantity': 'true'})
        unit_span = ingredient_li.find('span', attrs={'data-ingredient-unit': 'true'})
        name_span = ingredient_li.find('span', attrs={'data-ingredient-name': 'true'})
        ...
        if not quantity and not unit:
            return [name]
        quantity_unit = f"{quantity} {unit}".strip() if quantity and unit else quantity or unit
        return [quantity_unit, name]
...
# List of 15 recipe URLs to scrape from allrecipes.com
urls = [
